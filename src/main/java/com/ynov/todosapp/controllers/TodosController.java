...
public class TodosController {
    ...
    @GetMapping("")
    public ResponseEntity<?> retrieveTodos(@RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "20") int size) {
        if (size <= 0) {
            throw new InvalidPageSize();
        }

        TodosPaginedDTO todos = TodoMapper.todoPageToDTO(todoService.getAllTodos(page, size));
